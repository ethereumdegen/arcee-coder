...
The line number prefix format is: line number + tab. Everything after the tab is the actual file content to match. \
NEVER include line numbers or the tab prefix in old_string or new_string — only the file content itself.\n\
        ...
        if occurrences == 0 {
            ...
            return Ok(ToolOutput::error(format!(
                "old_string not found in {}. Common causes: \
                 (1) You included line numbers or the tab prefix from Read output — only include the actual file content. \
